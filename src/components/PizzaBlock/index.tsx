import React, { FC, useState } from 'react';

import classNames from 'classnames';

import { Nullable, PizzasType, ReturnComponentType } from 'types';
import { BIG_SIZE, MID_SIZE, SMALL_SIZE, THIN_TYPE } from 'vars';

export const PizzaBlock: FC<PizzasType> = ({
  name,
  types,
  imageUrl,
  price,
  sizes,
}): ReturnComponentType => {
  const availableTypes: string[] = ['тонкое', 'традиционное'];
  const availableSizes: number[] = [SMALL_SIZE, MID_SIZE, BIG_SIZE];
  const [activeType, setActiveType] = useState(types[THIN_TYPE]);
  const [activeSize, setActiveSize] = useState(sizes[SMALL_SIZE]);

  const onSelectType = (index: number): Nullable<void> => {
    setActiveType(index);
  };
  const onSelectSize = (index: number): Nullable<void> => {
    setActiveSize(index);
  };
  return (
    <div className="pizza-block-wrapper">
      <div className="pizza-block">
        <img className="pizza-block__image" src={imageUrl} alt="Pizza" />
        <h4 className="pizza-block__title">{name}</h4>
        <div className="pizza-block__selector">
          <ul>
            {availableTypes.map((type, index) => (
              <li
                onKeyDown={() => onSelectType(index)}
                role="menuitem"
                onClick={() => onSelectType(index)}
                key={type}
                className={classNames({
                  active: activeType === index,
                  disabled: !types.includes(index),
                })}
              >
                {type}
              </li>
            ))}
          </ul>
          <ul>
            {availableSizes.map((size, index) => (
              <li
                onKeyDown={() => onSelectSize(index)}
                role="menuitem"
                onClick={() => onSelectSize(index)}
                key={size}
                className={classNames({
                  active: activeSize === index,
                  disabled: !sizes.includes(size),
                })}
              >
                {size} см.
              </li>
            ))}
          </ul>
        </div>
        <div className="pizza-block__bottom">
          <div className="pizza-block__price">{price}</div>
          <div className="button button--outline button--add">
            <svg
              width="12"
              height="12"
              viewBox="0 0 12 12"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M10.8 4.8H7.2V1.2C7.2 0.5373 6.6627 0 6 0C5.3373 0 4.8 0.5373 4.8 1.2V4.8H1.2C0.5373 4.8 0 5.3373 0 6C0 6.6627 0.5373 7.2 1.2 7.2H4.8V10.8C4.8 11.4627 5.3373 12 6 12C6.6627 12 7.2 11.4627 7.2 10.8V7.2H10.8C11.4627 7.2 12 6.6627 12 6C12 5.3373 11.4627 4.8 10.8 4.8Z"
                fill="white"
              />
            </svg>
            <span>Добавить</span>
            <i>2</i>
          </div>
        </div>
      </div>
    </div>
  );
};
